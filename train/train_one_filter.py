# -*- coding: utf-8 -*-
"""
Created on Thu Mar  5 14:41:56 2020
...
import numpy as np
import model_one_filter as model
...
exp_path =  r'D:\Prateek\Experiments\{}-(Animation-56-Ventral-Step-1-Vary-LR-OneFilter-Start-Fixed)'.format(current_time)
...
for lr in lr_ls:
    ...
    log.write('Experiment Animation 3: One filter CNN all elman. BW data. Frames = max')
    log.write('Single filter, size = 9x9')
    log.write('\nLearning Rate:{}\n'.format(lr))
    log.close()
    ...
    #        print(video, video.split('_')[0])
            ...
            eloop_1 = np.ones([batch_size, 28, 28, 1])
            eloop_2 = np.ones([batch_size, 28, 28, 1])
            ...
            for image in image_list:
                ...
                eloop_1_, eloop_2_, cost, optimizer, output = sess.run([
                                                network['conv1_in'],
                                                network['conv2_in'],
                                                network['cost'],
                                                network['optimizer'],
                                                network['output']],
                                            feed_dict={network['image_input']: np.reshape(img, [batch_size, 28, 28, 1]),
    ...
    #        print(video, video.split('_')[0])
            ...
            for image in image_list:
                ...
                eloop_1_, eloop_2_, cost, output = sess.run([
                                                network['conv1_in'],
                                                network['conv2_in'],
                                                network['cost'],
                                                network['output']],
    ...
    #            os.makedirs(image_folder)
            ...
            for image in image_list:
                ...
                eloop_1_, eloop_2_, conv1_final_out, conv2_final_out, cost, output = sess.run([
                                                network['conv1_in'],
                                                network['conv2_in'],
                                                network['conv1_final_out'],
                                                network['conv2_final_out'],
                                                network['cost'],
                                                network['output']],
                                            feed_dict={network['image_input']: np.reshape(img, [batch_size, 28, 28, 1]),
                                                       network['conv1_eloop_input']: eloop_1,
                ...
                loss_per_video += cost
                first_layer_out.append(conv1_final_out)
                second_layer_out.append(conv2_final_out)
    ...
    plot_save_path = lr_path + '/images/'
    for k in range(len(first_layer_out)):    
        filters = 1
        plt.imshow(first_layer_out[k][0,:,:,0])
        plt.savefig(plot_save_path + name + '_' + str(k) + '.png')
        plt.close()
    
    name = 'second_layer_response'
    plot_save_path = lr_path + '/images/'
    for k in range(len(second_layer_out)):    
        filters = 1
        plt.imshow(second_layer_out[k][0,:,:,0])
        plt.savefig(plot_save_path + name + '_' + str(k) + '.png')
        plt.close()
